...
class ZFCalendarView: UIView {
    ...
    fileprivate let today: String = String(ZFDateTools.day(Date()))  //当天几号
    //原点
    ...
    fileprivate lazy var calendarCollectionView: UICollectionView = {
        ...
        calendarCollectionView.register(ZFDaysCell.self, forCellWithReuseIdentifier: self.identifier)
    ...
    func _initCalendarInfo() {
        //当前月份的总天数
        self.currentMonthTotalDays = ZFDateTools.daysInCurrMonth(date: date)
        
        //当前月份第一天是周几
        self.firstDayIsWeekInMonth = ZFDateTools.firstDayIsWeekInMonth(date: date)
        
        self.dateLabel.text = ZFDateTools.stringFromDate(date: date, format: "yyyy-MM")
        
        //是否当月
        let nowDate: String = ZFDateTools.stringFromDate(date: Date(), format: "yyyy-MM")
...
extension ZFCalendarView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: self.identifier, for: indexPath) as! ZFDaysCell
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let currentCell = collectionView.cellForItem(at: indexPath) as! ZFDaysCell
        ...
        let curDate = Date()
        let today = ZFDateTools.day(curDate)
        ...
        //获取上一次选中的item
        let preCell = collectionView.cellForItem(at: self.lastSelectedItemIndex!) as! ZFDaysCell
...
extension ZFCalendarView {
    @objc func lastAction() {
        self.date = ZFDateTools.lastMonth(date)
        self._initCalendarInfo()
        calendarCollectionView.reloadData()
    }
    
    @objc func nextAction() {
        self.date = ZFDateTools.nextMonth(date)
        self._initCalendarInfo()
        calendarCollectionView.reloadData()
    }
}
